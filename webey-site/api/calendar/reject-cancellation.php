 ...
 * POST { id } - Iptal talebini reddet -> randevu eski statusune doner
 */
...
try {
    ...
    try {
        ...
        $logRow = $logChk->fetch();
        if ($logRow && in_array((string)$logRow['prev_status'], ['approved', 'pending'], true)) {
            $prevStatus = (string)$logRow['prev_status'];
        }
    } catch (Throwable) {
    }

    $pdo->prepare('UPDATE appointments SET status = ?, updated_at = NOW() WHERE id = ? AND business_id = ?')
        ...
        if ($row) {
            ...
            if ($notifUserId) {
                ...
                wbInsertUserNotification(
                    ...
                    (int)$apptId,
                    $notif['type'],
                    'Iptal talebiniz reddedildi',
                    'Iptal talebiniz reddedildi. Randevunuz aktif durumda.',
                    (string)($row['business_name'] ?? '')
                );
            }
        }
    } catch (Throwable $mailEx) {
        error_log('[reject-cancellation mail] ' . $mailEx->getMessage());
    }
    ...
    try {
        ...
        if (!empty($aData['customer_phone'])) {
            queueSms($pdo, (string)$aData['customer_phone'], 'Webey: Iptal talebiniz reddedildi. Randevunuz devam etmektedir.', 'cancellation_rejected', $apptId);
        }
    } catch (Throwable $smsEx) {
        error_log('[reject-cancellation sms] ' . $smsEx->getMessage());
    }

    try {
        $pdo->prepare("UPDATE notifications SET result = 'cancel_rejected', is_read = 0 WHERE appointment_id = ? AND business_id = ? AND type = 'cancellation' AND result = 'pending'")
            ->execute([$apptId, $bid]);
    } catch (Throwable $nErr) {
        error_log('[reject-cancellation notif] ' . $nErr->getMessage());
    }

    wb_ok(['id' => (string)$apptId, 'status' => $prevStatus, 'message' => 'Iptal talebi reddedildi.']);
} catch (Throwable $e) {
